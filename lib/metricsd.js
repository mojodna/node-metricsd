...
module.exports = function(options) {
    ...
    var format = function(fmt, args) {
        if (!args || args.length === 0) {
            if (fmt instanceof Object) {
                ...
                return API.format.apply(null, [fmt[0], fmt.slice(1)]);
            }
        ...
        if (args[0] instanceof Object) {
            if (args[0].length > 0) {
                args[0] = API.format.apply(null, args[0]);
                prefixed = true;
            } else {
                args[0] = "";
            }
        ...
        // format the name
        var name = util.format.apply(null, args.slice(0, placeholders + 1));
    ...
    API.format = function(fmt, args) {
        args = Array.prototype.slice.call(arguments, 1);

        return format.apply(null, [fmt, args]);
    };

    API.deleteCounter = function(name) {
        API.write(API.format("%s:delete|c", arguments));
    };

    API.deleteGauge = function(name) {
        API.write(API.format("%s:delete|g", arguments));
    };

    API.deleteHistogram = function(name) {
        API.write(API.format("%s:delete|h", arguments));
    };

    API.deleteMeter = function(name) {
        API.write(API.format("%s:delete", arguments));
    };

    API.updateCounter = function(name, value) {
        // TODO sample rate
        API.write(API.format("%s:%s|c", arguments));
    };

    API.updateGauge = function(name, value) {
        API.write(API.format("%s:%s|g", arguments));
    };

    API.updateHistogram = function(name, value) {
        // TODO sample rate
        API.write(API.format("%s:%s|h", arguments));
    };
    ...
    Timer.prototype.stop = function(name) {
        name = API.format(arguments) || this.name;
    ...
    API.count = function(name) {
        try {
            return new Counter(API.format(name));
        } catch (e) {}
    };
